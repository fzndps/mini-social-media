document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (!resUser.ok)
      throw new Error(userData.data || "Gagal mengambil data user");
    ...
    if (!resPosts.ok)
      throw new Error(postData.data || "Gagal mengambil data postingan");

    const jumlahPostingan = postData.data.Posts.length;
    document.getElementById(
      "postCount"
    ).textContent = `${jumlahPostingan} postingan`;
    ...
    postData.data.Posts.forEach((post) => {
      ...
      div.innerHTML = `
        ...
        <div class="text-sm text-gray-500">
          ${post.CommentCount.Int32 ?? 0} komentar
        </div>
      `;
